    ...
    methods: {
      ...
      runMapFuc() {
        ...
        trackPoints[i] = new BMap.Point(
          this.firstRunList[i].longitude,
          this.firstRunList[i].latitude
        );
          // drowLine(this.map, this.firstArr,this.myIcon2)
        }
        drowLine(this.map, trackPoints);
        //画车
        const Carpoint = {
          lng:this.firstRunList[this.firstRunList.length-1].longitude,
          lat:this.firstRunList[this.firstRunList.length-1].latitude
        }
        this.drowCar(Carpoint,this.firstRunList[this.firstRunList.length-1].heading)
        ...
        function addMarker(point, name, mapInit, trackUnit) {
          if (name == "起点") {
            var myIcon = new BMap.Icon( require('@/assets/images/gpsImg/icon-start.png'), new BMap.Size(45, 45), {
      ...
      drowCar(point,heading) {
        var pt = new BMap.Point(point.lng,point.lat);
        let Icon = new BMap.Icon( require('@/assets/images/car/car0.png'), new BMap.Size(17,37));
    ...
    mounted() {
      ...
      this.getData('/gps/dynamic', 'post', {carId: this.$route.params.carId}).then(res => {
        ...
        if(this.firstRunList[0]){
          this.map.centerAndZoom(new BMap.Point(this.firstRunList[this.firstRunList.length-1].longitude, this.firstRunList[this.firstRunList.length-1].latitude), 15);
        }else{
          this.map.centerAndZoom(new BMap.Point(106.5065165683,29.6144641402), 15);
        }
        //地图中心点
