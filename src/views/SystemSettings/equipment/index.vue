      ...
      <div class="equipment-form">
        <el-form :inline="true" :model="equipmentForm" class="equipmentForm-inline">
          <el-form-item label="设备号">
            <el-input @keyup.native="inputChange(equipmentForm)" v-model="equipmentForm.din"
          ...
          <el-form-item label="新增日期" style="margin-right: 40px">
            <el-col :span="11">
              <el-date-picker type="date" placeholder="开始日期" v-model="equipmentForm.startDate"
                              style="width: 100%;"></el-date-picker>
            </el-col>
            <div class="line" :span="2" style="float: left;margin-left: 15px;margin-right: 9px">-</div>
            <el-col :span="11">
              <el-date-picker type="date" placeholder="结束日期" v-model="equipmentForm.stopDate"
                              style="width: 100%;"></el-date-picker>
            </el-col>
          </el-form-item>
          <el-form-item>
            <el-button size="mini" type="primary" @click.native="equipmentSearch(equipmentForm)">查询</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
       ...
       */
      resetForm(form) {
        this.$refs.mineOrgName.val.orgValue = ''
      ...
      deviceSureFuc(){
        /* 判断是否批量调拨 */
        if(this.ids){
          this.getData('/device/batchTransfer', 'post',{ids:this.ids,orgId: this.$refs.upOrgNameDevice.val.id}, true).then(res => {
            if(res.code===0){
