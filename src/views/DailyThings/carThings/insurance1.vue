<template>
  <div class="insurance1-thing">
    <div class="insurance-thing-form">
      <el-form :inline="true" :model="carInsurance1Form" class="carInsurance1Form-inline">
        <el-form-item label="车牌号">
          <el-input @keyup.native="inputChange(carInsurance1Form)" v-model="carInsurance1Form.license"
                    placeholder="请输入车牌号"></el-input>
        </el-form-item>
        <el-form-item label="所属机构/部门" class="orgBox">
          <jt-org-select ref="insurance1Org" @orgCurrent="orgCurrent1"></jt-org-select>
        </el-form-item>
        <el-form-item label="投保日期">
          <el-date-picker
            class="range"
            style="width: 230px;margin-right: 30px"
            v-model="carInsurance1Form.doDate"
        ...
        <el-form-item>
          <el-button size="mini" type="primary" @click="carInsuranceSearch(carInsurance1Form)">查询</el-button>
          <el-button class="fn-btn-reset" size="mini" @click="resetForm(carInsurance1Form)">重置</el-button>
        </el-form-item>
        <div>
          <el-form-item>
    ...
    <div class="carInsuranceTable">
      <jt-table ref="carInsurancTable" @tableSelectionChange="tableSelectionChange" :tableData="carInsurance1TableData">
        <div slot="tablePage">
    ...
    <jt-dialog :dialog-show.sync="dialogShow" :title="dialog.title" :width="dialog.width">
      <div slot="dialog">
        <el-form label-position="right" ref="insurance1AddForm" :model="insurance1AddForm" id="insurance1AddForm">
          <!--:rules="rules"-->
          <el-row>
            <el-col :span="12">
              <div class="grid-content bg-purple">
                <el-form-item label="车牌号" prop="license">
                  ...
                  <el-select
                    style="width: 193px;"
                    v-model="insurance1AddForm.carId"
                    filterable
                    remote
                    @change="carChange"
                    reserve-keyword
                ...
                <el-form-item label="投保日期" prop="doDate">
                  <el-date-picker
                    v-model="insurance1AddForm.doDate"
                    type="date"
                    placeholder="投保日期"
                ...
                <el-form-item label="备注" prop="remark">
                  <el-input v-model="insurance1AddForm.remark" placeholder="请输入备注信息" style="width: 193px;"></el-input>
                </el-form-item>
                <el-row>
                </el-row>
              </div>
            ...
            <el-col :span="12">
              <div class="grid-content bg-purple-light">
                ...
                <el-form-item label="缴费金额" prop="doAmount">
                  <el-input v-model="insurance1AddForm.doAmount" placeholder="请输入金额" style="width: 193px;"></el-input>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-form-item style="text-align: center;width: 100%;margin-top: 20px">
            ...
            <el-button size="mini" type="primary" @click.native="insuranceAddFuc(insurance1AddForm)">确 定</el-button>
          </el-form-item>
        </el-form>
      </div>
    </jt-dialog>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        carInsurance1TableData: {
            ...
            },
            {
              ...
              handleFunc: (obj) => {
                this.$confirm('此操作将删除该保险, 是否继续?', '提示', {
                  ...
                  this.getData('/insure/delete', 'post', {id: obj.row.id, insureType: 2}, true).then(res => {
                    if (res.code === 0) {
                      this.getPageFuc()
                    }
                  })
          ...
          upload: true,
          templateUrl: '/template/insurance1_model.xlsx',
    ...
    methods: {
      ...
      carChange(val) {
        for (var i = 0, len = this.carOption.length; i < len; i++) {
          if (val === this.carOption[i].id) {
            ...
            this.insurance1AddForm.orgId = this.carOption[i].orgId
          }
        }
      },
      /**
       ...
       */
      carInsuranceSearch() {
        if(this.carInsurance1Form.doDate){
          this.carInsurance1Form.startTime=this.carInsurance1Form.doDate[0]
          this.carInsurance1Form.endTime=this.carInsurance1Form.doDate[1]
        }
        this.getPageFuc()
      },
      /**
       ...
       */
      resetForm() {
        this.$refs.insurance1Org.val.orgValue = ''
        this.carInsurance1Form = {
          license: '',
          doDate: '',
          startTime:'',
          endTime:'',
       ...
       */
      orgCurrent1(val) {
        this.carInsurance1Form.orgId = val.id
      },
      /**
       * 弹窗中的组织机构选择
       * @param val 组织机构id
       */
      orgCurrent2(val) {
        this.insurance1AddForm.orgId = val.id
      },
      /**
       * 打开新增弹窗-按钮
       */
      ...
      handleSizeChange(val) {
        this.carInsurance1Form.pageSize = val
        this.getPageFuc();
      },
      /**
       * 分页-当前页
      ...
      changeCurrentPage(val) {
        this.carInsurance1Form.pageNum = val
        this.getPageFuc();
      },
      /**
       * 新增，编辑确定方法
      ...
      insuranceAddFuc() {
        // insure/add
        this.insurance1AddForm.doDate = datetime(this.insurance1AddForm.doDate)
        if (this.update) {
          this.getData('/insure/update', 'post', this.insurance1AddForm, true).then(res => {
            if (res.code === 0) {
              this.getPageFuc();
          ...
          this.getData('/insure/add', 'post', this.insurance1AddForm, true).then(res => {
            if (res.code === 0) {
              this.getPageFuc();
              this.dialogShow = false
            }
       ...
       */
      getPageFuc() {
        this.getData('/insure/page', 'post', this.carInsurance1Form).then(res => {
          if (res.code === 0) {
            this.pager.total = res.data.total
            res.data.list.forEach((ele) => {
              ele.doDate = datetime(ele.doDate)
            })
            this.carInsurance1TableData.list = res.data.list
          }
        }).catch(error => {
          console.log(error)
        });
