    ...
    <div class="insurance-remind-form">
      <el-form :inline="true" :model="carInsuranceForm" class="carInsuranceForm-inline">
        ...
        <div>
          ...
          <el-form-item>
            <el-button size="mini" icon="el-icon-message" type="warning" @click="addInsuranceMsg">{{subscribe}}</el-button>
          </el-form-item>
        </div>
      </el-form>
    </div>
...
<script>
  ...
  export default {
          ...
          downList: [
            {
              ...
              handleFunc: () => {
                this.$confirm('此操作将全部提醒消息标记为已读，是否继续', '提示', {
                  ...
                  this.getData('/cw/remind/allSignRead', 'post',{allRead:true, insureType:2},true).then(res => {
                    if(res.code===0){
                      this.getPage()
                    }
                  }).catch(error => {
    ...
    methods: {
      ...
      addInsuranceMsg(){
        if(this.subscribe==='未订阅消息'){
          this.$confirm('此操作将订阅消息', '提示', {
            ...
            this.getData('/cw/condition/subscribe', 'post',{issubscribe:1,insureType:2},true).then(res => {
              if(res.code===0){
          ...
          this.$confirm('此操作将取消订阅消息, 是否继续?', '提示', {
            ...
            this.getData('/cw/condition/subscribe', 'post',{issubscribe:0,insureType:2},true).then(res => {
              if(res.code===0){
      ...
      conditionDay(){
        this.getData('/cw/condition/get ', 'post', {insureType: 2}).then(res => {
          if (res.code === 0) {
